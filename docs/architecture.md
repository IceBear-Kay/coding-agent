...
- **Document Parser**：在工作区边界内有界读取文本型 PDF 和简单 DOCX；解析在可终止的受控进程中进行，结果以结构化只读工具消息返回。
...
- `read_document` 使用 `pypdf` 提取 PDF 页面文字，使用 `python-docx` 按正文顺序提取 DOCX 段落和简单表格。源文件最多 5 MiB，PDF 单次最多 20 页，文本最多 32000 个字符；损坏、加密、无文字层、不支持格式、路径越界和资源超限会返回结构化错误，工具不会修改原文档。
