...
文档读取受固定资源限制：单个源文件最多 5 MiB，PDF 单次最多 20 页，提取文本最多 32000 个字符，解析结果最多 256 KiB。页码从 1 开始且拒绝布尔值；`processed_end_page` 表示实际处理到的末页，文本上限提前触发时会小于计划范围。选定页面没有文字时只说明该范围，`no_text` 结果仍会标记未读取页或其他截断原因，不推断整份 PDF 为空。解析进程只接收已校验的有界字节并使用最小化环境，不继承模型密钥等无关凭据。仅支持文本型 PDF 和简单 DOCX；不提供 OCR、图片理解、旧版 `.doc`、`.docm`、宏执行或复杂版式还原。损坏、加密、无文字层、越界路径和资源超限会返回结构化错误，原文档保持不变。
...
输入 `/sessions` 可按更新时间查看当前工作区的会话并通过编号恢复，回车取消；输入 `/rename <标题>` 可在不调用模型的情况下修改当前会话标题。新持久会话的首个有效任务会进行一次受限标题请求，失败时使用本地标题，不影响主任务。

聊天输入提示为“输入指令：”，可使用 `/help` 查看会话命令。默认显示模型给出的简短行动说明和工具进度；`--no-show-plan` 只隐藏中间说明，不影响最终答案、工具调用、审批或错误。真实交互终端会使用 Rich 渲染 Markdown，重定向或非 TTY 环境自动使用纯文本。
...
持久聊天可使用 `--session ID` 新建或 `--resume ID` 恢复，并可用 `--session-dir PATH` 指定存档目录；省略 `--session-dir` 时默认使用启动目录下的 `.local/sessions`。省略位置任务时的默认聊天同样会创建新的持久会话；如需纯内存聊天请显式使用 `--no-save`。`--session` 与 `--resume` 互斥，且必须处于聊天模式（显式使用 `--chat` 或省略位置任务进入默认聊天）。持久会话保存完整历史，不保存裁剪后的请求上下文、Provider 配置、环境变量、API Key、审批状态或进程对象；存档目录受到工具访问保护，不会被 Agent 文件工具读取或写入。
...
| `--session-dir PATH` | 启动目录下 `.local/sessions`（聊天模式） | 指定会话 JSON 存档目录；聊天模式可单独指定，`--no-save` 不能使用。 |
...
| `--show-plan` / `--no-show-plan` | 默认显示 | 控制模型伴随工具调用的简短行动说明；隐藏时不改变请求、工具、审批或最终答案。 |
...
- `--chat` 默认将正常完成任务的历史保存到 JSON 存档并跨进程恢复；使用 `--no-save` 可显式改为仅内存模式。`/sessions` 可列出并选择当前工作区的存档，`/rename <标题>` 只修改会话元数据。`max_steps`、Provider 错误和非正常模型停止原因会结束整个会话，不继续使用不完整历史。
