...
`coding-agent` 是一个在本地工作区运行的 Coding Agent：模型读取任务和文件，按需请求本地工具，并根据真实工具结果继续处理。当前支持读取 UTF-8 文本文件、经用户批准创建和精确修改文件，以及经用户批准执行本地命令。
...
| 参数 | 默认值 | 作用与限制 |
| --- | --- | --- |
| `task` | 省略时交互输入 | 一次任务文本。每次启动只处理一个任务。 |
...
| `--max-steps` | `8` | Provider 调用次数上限，必须为正整数；临时错误重试也计入。不是工具调用次数上限。 |
| `--max-retries` | `2` | 每次临时 Provider 错误的最大重试次数，必须为非负整数；设为 `0` 关闭自动重试。 |

`--max-steps` 统计模型请求次数；一次模型响应可以包含多个工具调用，这些调用按模型给出的顺序处理。写入和执行工具仍按调用逐次审批。`DEEPSEEK_TIMEOUT_SECONDS` 控制 API 网络请求，与 `--command-timeout` 控制的本地子进程超时无关。

## 审批与限制

- 写入和本地命令执行都必须逐次审批。输入 `y` 或 `yes` 才批准；直接回车、其他输入、EOF 或非交互管道输入都会拒绝。
- `Ctrl+C` 中断当前任务并返回退出码 `130`。工具结果会先写入任务历史，再显示给终端。
- 正常结束只表示模型不再请求工具，不等于生成的程序通过了独立测试；命令的退出码 `0` 也不自动证明答案正确。
- 每次启动处理一个任务，当前没有跨启动会话恢复或上下文压缩。
