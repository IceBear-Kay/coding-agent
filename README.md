...
持久聊天可使用 `--session ID` 新建或 `--resume ID` 恢复，并可用 `--session-dir PATH` 指定存档目录；省略 `--session-dir` 时默认使用启动目录下的 `.local/sessions`。`--session` 与 `--resume` 互斥，且只能与 `--chat` 一起使用。普通 `--chat` 不会创建存档。持久会话保存完整历史，不保存裁剪后的请求上下文、Provider 配置、环境变量、API Key、审批状态或进程对象；存档目录受到工具访问保护，不会被 Agent 文件工具读取或写入。
...
| `--session ID` | 未启用 | 新建指定 ID 的持久聊天会话；只能与 `--chat` 一起使用，不能与位置任务同时使用。 |
| `--resume ID` | 未启用 | 恢复指定 ID 的持久聊天会话；只能与 `--chat` 一起使用，不能与位置任务同时使用。 |
